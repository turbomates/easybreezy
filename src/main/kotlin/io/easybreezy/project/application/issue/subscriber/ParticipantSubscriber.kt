...
import io.easybreezy.project.application.issue.command.ApplyParticipants
import io.easybreezy.project.application.issue.command.Handler

class ParticipantSubscriber @Inject constructor(private val handler: Handler) : EventsSubscriber {
        ...
        return listOf(
            Created to object : EventSubscriber<Created> {
                override suspend fun invoke(event: Created) {
                    if (null !== event.description) {
                        handler.applyParticipants(ApplyParticipants(event.issue, event.description))
                    }
                }
            },
            ...
            Commented to object : EventSubscriber<Commented> {
                override suspend fun invoke(event: Commented) {
                    handler.applyParticipants(ApplyParticipants(event.issue, event.content))
                }
            }
        )
    }
