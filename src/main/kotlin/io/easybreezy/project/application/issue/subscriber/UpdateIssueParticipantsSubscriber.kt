...
import io.easybreezy.project.application.issue.command.UpdateParticipants
import io.easybreezy.project.application.issue.command.Handler

class UpdateIssueParticipantsSubscriber @Inject constructor(private val handler: Handler) : EventsSubscriber {
        ...
        return listOf(
            Created to object : EventSubscriber<Created> {
                override suspend fun invoke(event: Created) {
                    if (null !== event.description) {
                        handler.updateParticipants(UpdateParticipants(event.issue, event.project, event.description))
                    }
                }
            },
            ...
            Commented to object : EventSubscriber<Commented> {
                override suspend fun invoke(event: Commented) {
                    handler.updateParticipants(UpdateParticipants(event.issue, event.project, event.content))
                }
            }
        )
    }
