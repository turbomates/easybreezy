...
import io.easybreezy.project.application.issue.command.UpdateTiming
import io.easybreezy.project.application.issue.command.Handler

class UpdateIssueTimingSubscriber @Inject constructor(private val handler: Handler) : EventsSubscriber {
        ...
        return listOf(
            Created to object : EventSubscriber<Created> {
                override suspend fun invoke(event: Created) {
                    if (null !== event.description) {
                        handler.updateTiming(UpdateTiming(event.issue, event.project, event.description))
                    }
                }
            },
            ...
            Commented to object : EventSubscriber<Commented> {
                override suspend fun invoke(event: Commented) {
                    handler.updateTiming(UpdateTiming(event.issue, event.project, event.content))
                }
            }
        )
    }
