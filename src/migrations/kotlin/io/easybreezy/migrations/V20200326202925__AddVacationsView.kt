    ...
    override fun migrate(context: Context) {
        context.execute("""
            ...
            (SELECT count(*) /*absence days without holidays*/
                ...
                WHERE a.user_id = ul.user_id and extract('ISODOW' FROM a_day) < 6
            )
            -
            ...
            (SELECT count(*) FROM holidays h /* missed working days */
                LEFT JOIN calendars c on h.calendar = c.id and l.id = c.location
                LEFT JOIN absences a on a.user_id = ul.user_id
                WHERE h.day between a.started_at and a.ended_at and h.is_working_day = true)
            as absence_days,
            ...
            LEFT JOIN working_hours wh on ul.user_id = wh.user_id
