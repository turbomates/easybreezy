...
class V20200413141310__AlterUserLocEndedAt : BaseJavaMigration() {
    override fun migrate(context: Context) {
        context.execute(
            """
            alter table user_locations alter column ended_at drop not null;
        """.trimIndent()
        )
        context.execute("""
            drop view vacations;
        ...
        context.execute("""
            ...
            (SELECT count(*) /*absence days without holidays*/
                ...
                WHERE a.is_approved = true and a.user_id = ul.user_id and extract('ISODOW' FROM a_day) < 6
                  and a.started_at >= ul.started_at and a.ended_at <= COALESCE(ul.ended_at, now())
            )
            -
            (SELECT count(*) FROM holidays h /* holidays on working days */
                LEFT JOIN calendars c on h.calendar = c.id and l.id = c.location
                LEFT JOIN absences a on a.user_id = ul.user_id and a.is_approved = true
            ...
            LEFT JOIN working_hours wh on ul.user_id = wh.user_id and wh.day between ul.started_at and COALESCE(ul.ended_at, now())
